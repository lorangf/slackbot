...
class ServiceRegistry {
  constructor() {
    this._services = []; // private variable
    this._timeout = 30; // timeout in seconds
  }
  ...
  add(intent, ip, port) {
    ...
    if (!this._services[key]) {
      ...
      console.log(`Add service for ${intent} on ${ip}:${port}`);
      this._cleanup();
      return;
    }

    this._services[key].timestamp = Math.floor(new Date()/1000);
    console.log(`Update service for ${intent} on ${ip}:${port}`);
    this._cleanup();
  }
  ...
  _cleanup() {
    ...
    for(let key in this._services) {
      if ( this._services[key].timestamp + this._timeout < now ) {
        console.log(`Remove service for intent ${this._services[key].intent}`);
        delete this._services[key];
      }
    }
  }
